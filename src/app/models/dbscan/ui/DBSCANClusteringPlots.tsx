...
import type { DBSCANClusteringTrainingReport, DBSCANSettings } from '../types';
...
type DBSCANPlotsProps = {
    dataset: Dataset;
    report: DBSCANClusteringTrainingReport;
    modelSettings: DBSCANSettings;
};

export function DBSCANClusteringPlots({ dataset, report, modelSettings }: DBSCANPlotsProps) {
    ...
    const hasAssignments = report.type === 'dbscan' && (trainAssignments?.shape[0] ?? 0) > 0;
    ...
    const data2D = useDBSCANPlotData({
        trainInputFeatures,
        testInputFeatures,
        trainPredictions: trainAssignments,
        testPredictions: testAssignments,
        numClusters,
        activePointIndex,
        hasAssignments,
        getColor,
