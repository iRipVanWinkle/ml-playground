...
import { TRANSFORMATIONS } from '../model/constants';
import type { TransformationType } from '../model/types';
import { useTransformations } from '../model/hooks';

export type TransformationsProps = {
    numFeatures: number;
    disabled?: boolean;
};

export function Transformations({ numFeatures, disabled }: TransformationsProps) {
    const [transformations, onChange] = useTransformations();
    ...
    const handleNewTransformation = () => {
        const updatedTransformations = [
            ...
            { type: '' as TransformationType, degree: 1 },
        ];
    ...
    const handleUpdateType = (index: number, value: string) => {
        const updatedTransformations = [...localTransformations];
        updatedTransformations[index].type = value as TransformationType;
        setLocalTransformations(updatedTransformations);
        onChange(updatedTransformations);
    };
